...
{
	InitializeDX( origIDirect3DDevice9 );
	render->Invalidate();
...
{	
	...
	InitializeDX( origIDirect3DDevice9 );
	g_WindowHandle = pPresentationParameters->hDeviceWindow;
	bD3DRenderInit = true;
}
...
HRESULT proxyIDirect3DDevice9::Reset ( D3DPRESENT_PARAMETERS *pPresentationParameters )
{
	...
	// now, but NFS should be handling this on its own, so whatever.
										 ...
										 CONST RGNDATA *pDirtyRegion )
{
	...
	if ( isBeginRenderWIN )
	{
		FrameTick( origIDirect3DDevice9, pPresentParam.hDeviceWindow );
		render->EndRender();
	}
